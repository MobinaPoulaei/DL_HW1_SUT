...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #############################################################################
  if use_loop:
    z = torch.stack([x[i].mm(y[i]) for i in range(x.size(0))], dim=0)
  else:
    z = x.bmm(y)
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def normalize_columns(x):
  ...
  #############################################################################
  m, n = x.shape
  mu = x.sum(dim=0)/m # shape: (3, )
  std = torch.sqrt((x - mu)**2).sum(dim=0)/(m - 1)
  y = (x - mu)/std # shape: (3, 3)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
